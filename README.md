...
质量检查用于发现风险，不替代作者判断。自动生成未通过时，正文会保存为“问题稿”，不会丢失；作者仍可直接保存为最终稿。手动保存只拒绝空正文，其他问题均以提醒形式保留，不会禁用保存按钮。

修订稿如果没有通过自动验收，会保留为候选稿并直接载入编辑器。作者可以继续修改、再次修订，或者直接确认保存为最终稿。

记忆只能根据已经显式保存的最终稿生成。系统会在记忆返回后再次核对正文版本和内容哈希；如果正文在生成期间发生变化，本次记忆不会入库，也不会删除任何版本。记忆成功后，系统清空该章草稿、候选版本、旧审稿结果和运行日志中的正文副本，只保留当前 `final_text`、最新记忆以及不含正文的调用统计。
