...
记忆卡只服务下一章顺利承接，避免写成剧情预测或营销分析：人物处于什么状态，冲突停在哪里，哪些问题还没解决，哪些内容下一章禁止跳过。

记忆卡同时要为下一章的 `minimal_memory_pack` 提供数据源。只留下“下一章不知道就会写错”的信息，避免把本章所有细节都塞进去。

## 记忆规则

- 不要把推测、未来剧情安排、隐藏解释或细纲计划写成本章事实。
- 不要复制细纲或内部协议里的方括号标签，例如“【目的词】”“【回报类型】”“【章首钩子】”“【章尾钩子】”“【强度】”。`summary`、`ending_hook` 和 `handoff` 都必须是可直接给下一章使用的自然语言事实。
...
- 接力棒是章节交接口，避免写成“氛围总结”：上一章最后停在哪个外部动作、哪句对白、哪个物件/证据、哪个未闭合冲突上，下一章第一段就必须从这里接住。
...
- `ending_hook`：本章结尾留下的具体钩子，只写自然语言事实，不要写内部标签或提纲格式。
