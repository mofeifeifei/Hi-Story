...
class ReviewerAgent(BaseAgent):
    ...
    def review_chapter(self, context: dict[str, Any], draft: str) -> dict[str, Any]:
        ...
        user_prompt = (
            ...
            "请额外给出 title_decision：先用一句话概括谁做了什么、造成什么变化，再给出 4 到 6 个不同结构的自然中文标题，"
            "从中选出 recommended_title 并说明 reason。标题必须概括本章主要回报，不能只摘地点、物件或人物名，"
            "也不能写成公文状态、数据库标签或名词堆叠；避免连续“X之Y”、抽象概念词和近章重复结构。\n\n"
            f"{history_section}\n"
            f"上下文：\n{json_dumps(context)}\n\n"
