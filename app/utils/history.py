...
def history_prompt_section(context: dict[str, Any], *, task: str = "writer") -> str:
    ...
    if task == "reviewer":
        return (
            ...
            "- 只指出会影响可信度的问题，不要要求牺牲剧情张力。当前章节相关事实在 historical_constraints 中。\n"
        )
    if task == "reviser":
        return (
            "【历史修订边界】\n"
            "- 修订语言和细节时，不得改动锁定历史事实、官职称谓、制度规则和时代背景。\n"
            "- 如需修正历史违和词，只替换表达，不重启剧情。当前章节相关事实在 historical_constraints 中。\n"
        )
    if task == "memory":
        return (
            "【历史记忆要求】\n"
    ...
    if task == "writer":
        return (
            "【历史写作边界】\n"
            "- 当前章节需要的历史事实已在上下文的 historical_constraints 中，只使用其中相关内容。\n"
            "- 不出现现代词、后世制度、超时代交通通信或错误称谓；未锁定的支线可为剧情服务。\n"
        )
    return (
        "【历史写作约束】\n"
