...
export function WritingPage() {
  ...
  const workState = useQuery({
    ...
    queryFn: () => api<WorkState>(`/api/works/${store.selectedWorkId}`),
  ...
  function draftKey(chapterNumber = store.selectedChapter) { return `hi-story:draft:${store.selectedWorkId}:${chapterNumber}`; }
  function scrollKey(chapterNumber = store.selectedChapter) { return `hi-story:scroll:${store.selectedWorkId}:${chapterNumber}`; }
  function expandedKey() { return `hi-story:writing-expanded:${store.selectedWorkId}`; }
  function saveLocalDraft() {
    if (!editor || !store.selectedChapter) return;
    try { localStorage.setItem(draftKey(), JSON.stringify({ text: plainText(), title, updatedAt: chapterState.data?.chapter.updated_at || "", savedAt: Date.now() })); } catch { /* Browser storage can be disabled. */ }
  }
  function restoreLocalDraft(chapter: Chapter) {
    try {
      ...
      if (!draft?.text || draft.text === plainText()) return;
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const chapter = chapterState.data!.chapter;
      return api<ChapterState>(`/api/works/${store.selectedWorkId}/chapters/${chapter.chapter_number}`, {
        method: "PUT",
    ...
    onSuccess: (data) => {
      queryClient.setQueryData(["chapter", store.selectedWorkId, store.selectedChapter], data);
      queryClient.invalidateQueries({ queryKey: ["work", store.selectedWorkId] });
      localStorage.removeItem(draftKey());
      setCandidateText(null); setDirty(false); setSavedAt(new Date().toLocaleTimeString("zh-CN", { hour12: false, hour: "2-digit", minute: "2-digit" }));
      store.notify("最终稿已保存。", "success");
    },
  ...
  async function runTask(kind: "chapter" | "revise" | "memory") {
    ...
    try {
      ...
      if (kind === "revise" && data.candidate_only && data.revised_text && targetStillActive) {
        setCandidateText(data.revised_text); setInspectorTab("versions");
        store.notify("修订稿已保存为候选版本，未覆盖最终稿。", "warning");
      } else {
        ...
        if (targetStillActive) setDirty(false);
  ...
  async function loadContext() {
    if (!store.selectedChapter || loadingContext || context) return;
    setLoadingContext(true);
    try {
      const data = await api<any>(`/api/works/${store.selectedWorkId}/chapters/${store.selectedChapter}/context`);
  ...
  useEffect(() => { if (inspectorTab === "continuity") loadContext(); }, [inspectorTab, store.selectedChapter]);
  ...
  const chapter = chapterState.data?.chapter;
  const reviewText = chapterState.data?.review_readable || readable(chapterState.data?.review);
  const versions = chapterState.data?.candidate_versions || [];
  const canSaveFinal = Boolean(
    chapter
    && plainText().trim()
    && (dirty || chapter.status === "problem_draft" || chapter.status === "draft")
  ...
  function loadCandidate(text: string) {
    ...
    store.notify("候选稿已载入编辑器，确认后可保存为最终稿。", "info");
  }
  ...
  async function deleteCandidate(versionId: unknown) {
    if (!chapter || !versionId || !window.confirm("确定删除这份候选稿吗？此操作无法撤销。")) return;
    try {
  ...
  return <div className="page">
    <PageHeader title="正文写作" description="章节独立加载，本地草稿与阅读位置自动保留。" actions={<>
      ...
      <button className="btn" disabled={!chapter || Boolean(store.task)} onClick={() => runTask("memory")}><FileClock size={16} />记忆入库</button>
    ...
    <div className={`writing-shell ${store.inspectorOpen ? "" : "inspector-closed"}`}>
      ...
      <section className="editor-column">{chapterState.isLoading ? <div className="loading-block">正在载入章节...</div> : chapter ? <>
        <div className="editor-toolbar"><span className="chapter-number">第 {chapter.chapter_number} 章</span><input className="title-input" value={title} onChange={(event) => { setTitle(event.target.value); setDirty(true); }} placeholder="章节标题" /><span className={`status-badge ${chapter.status || ""}`}>{statusText(chapter.status)}</span></div>
      ...
      <aside className="inspector"><div className="inspector-tabs">{([['task','本章任务'],['continuity','上下章衔接'],['revision','修订'],['memory','记忆'],['versions','候选版本']] as Array<[InspectorTab,string]>).map(([key,label]) => <button key={key} className={inspectorTab === key ? "active" : ""} onClick={() => setInspectorTab(key)}>{label}</button>)}<button className="inspector-close" onClick={store.toggleInspector} title="关闭辅助面板"><X size={16} /></button></div><div className="inspector-content">
        ...
        {inspectorTab === "revision" && <div className="revision-box"><h3>修订建议</h3><pre className="readable">{reviewText || "当前正文没有可用的修订建议。"}</pre><button className="btn" disabled={!reviewText.trim()} onClick={copyRevisionText}><Copy size={15} />复制修改话术</button><label className="field">修改要求<textarea value={instruction} onChange={(e) => setInstruction(e.target.value)} placeholder="粘贴上方修改话术，或直接写清楚要改什么。" /></label><button className="btn primary" disabled={!instruction.trim() || Boolean(store.task)} onClick={() => runTask("revise")}><Play size={15} />按要求修订</button></div>}
        ...
        {inspectorTab === "versions" && <><h3>候选版本</h3>{candidateText && <Candidate text={candidateText} label="本次修订候选稿" onPreview={() => setPreviewCandidate({ text: candidateText, label: "本次修订候选稿" })} onLoad={() => loadCandidate(candidateText)} onPromote={() => promoteCandidate(candidateText)} />}{versions.map((version) => { const text = String(version.content || ""); return <Candidate key={String(version.id)} text={text} label="历史候选稿" onPreview={() => setPreviewCandidate({ text, label: "历史候选稿" })} onLoad={() => loadCandidate(text)} onPromote={() => promoteCandidate(text)} onDelete={() => deleteCandidate(version.id)} />; })}{!candidateText && !versions.length && <p className="readable">暂无候选版本。未通过自动验收的稿件会保存在这里，不会覆盖最终稿。</p>}</>}
      </div></aside>
    </div>
    {previewCandidate && <div className="modal-backdrop" role="presentation" onMouseDown={() => setPreviewCandidate(null)}><section className="candidate-preview" role="dialog" aria-modal="true" aria-label="候选稿预览" onMouseDown={(event) => event.stopPropagation()}><header><div><strong>{previewCandidate.label}</strong><span>候选稿，未覆盖最终稿</span></div><button className="icon-button" title="关闭预览" onClick={() => setPreviewCandidate(null)}><X size={17} /></button></header><div className="candidate-preview-body">{previewCandidate.text}</div><footer><button className="btn" onClick={() => setPreviewCandidate(null)}>关闭</button><button className="btn" onClick={() => loadCandidate(previewCandidate.text)}><Eye size={15} />载入编辑器</button><button className="btn primary" disabled={saveMutation.isPending} onClick={() => promoteCandidate(previewCandidate.text)}><Save size={15} />保存为最终稿</button></footer></section></div>}
  </div>;
}

function Candidate({ text, label, onPreview, onLoad, onPromote, onDelete }: { text: string; label: string; onPreview: () => void; onLoad: () => void; onPromote: () => void; onDelete?: () => void }) { return <div className="candidate"><div className="candidate-head"><strong>{label}</strong><span className="status-badge problem_draft">未覆盖最终稿</span></div><p>{text}</p><div className="candidate-actions"><button className="btn small" onClick={onPreview}><Eye size={14} />预览</button><button className="btn small" onClick={onLoad}><Play size={14} />载入</button><button className="btn small" onClick={onPromote}><Save size={14} />转为最终稿</button>{onDelete && <button className="icon-button danger-icon" title="删除候选稿" onClick={onDelete}><Trash2 size={14} /></button>}</div></div>; }
